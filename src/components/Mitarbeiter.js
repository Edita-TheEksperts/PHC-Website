                  ...
                  {employeeVacations.map((v) => (
                    <li key={v.id} className="p-4 border rounded-xl bg-white shadow-sm">
                      ...
                      <div className="flex flex-wrap gap-2 mb-2">
                        {v.employee?.phone && (
                          <button onClick={() => window.open(`tel:${v.employee.phone}`)} className="px-3 py-1 bg-blue-500 text-white text-xs rounded-lg hover:bg-blue-600">
                            Anruf
                          </button>
                        )}
                      ...
                      {v.suggestions && v.suggestions.length > 0 && (
                        <div className="bg-gray-50 p-2 rounded-lg border text-xs">
                          <p className="font-semibold text-gray-700 mb-1">💡 Alternativen:</p>
                          <ul className="space-y-2">
                            {v.suggestions.map((s, i) => (
                              <li key={i} className="flex justify-between items-center p-2 bg-white border rounded-lg">
                                <div>
                                  <p>{new Date(s.startDate).toLocaleDateString()} → {new Date(s.endDate).toLocaleDateString()}</p>
                                  <p className="font-medium">👷 {s.employee.firstName} {s.employee.lastName}</p>
                                </div>

                                <div className="flex gap-2">
                                  {s.employee.phone && (
                                    <button onClick={() => window.open(`tel:${s.employee.phone}`)} className="px-2 py-1 bg-blue-500 text-white text-[10px] rounded-lg hover:bg-blue-600">
                                      📞
                                    </button>
                                  )}
                                  ...
                                  <button
                                    onClick={async () => {
                                      await fetch("/api/admin/vacation/assign", {
                                        method: "PATCH",
                                        headers: { "Content-Type": "application/json" },
                                        body: JSON.stringify({ vacationId: v.id, newEmployeeId: s.employee.id }),
                                      });
